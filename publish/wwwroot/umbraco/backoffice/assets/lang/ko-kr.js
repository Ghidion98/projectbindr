export default {
    ...
    defaultdialogs: {
        ...
        regexSearchError: "<a target='_blank' rel='noopener' href='http://regexlib.com'>regexlib.com</a>의 웹서비스는 현재 제어할 수 없는 몇가지 문제점이 보고되었습니다. 불편을 드려 대단히 죄송합니다.",
    ...
    dictionaryItem: {
        description: "'%0%'사전 항목 아래에 다른 언어버전들을 편집하세요<br/>왼쪽 '언어'메뉴를 사용하여 추가 언어들을 설정할 수 있습니다.",
        displayName: '국가명',
    },
    editdatatype: {
        addPrevalue: '이전값 더하기',
    ...
    installer: {
        ...
        databaseText: '이과정을 위해선, 당신의 DB서버 정보에 대해서 알고 계셔야합니다.("connection string").<br />\n      필요하시다면 사용하시는 ISP쪽에 문의하시기 바랍니다..\n      로컬 머신이나 서버에 설치되어 있다면 해당 시스템 관리자에게 문의하시기 바랍니다.',
        databaseUpgrade: '<p><strong>업그레이드</strong> 버튼을 누르면 여러분의 데이터베이스를 Umbraco %0% 로 업데이트합니다.</p><p>어떤 컨텐트도 삭제되지 않으니 걱정마세요!</p>',
        databaseUpgradeDone: '데이터베이스가 최신 버전 %0% 로 업그레이드 되었습니다.<br />계속 진행하시려면 <strong>다음</strong> 을 누르세요. ',
        databaseUpToDate: '데이터베이스가 업데이트 되었습니다. <strong>다음</strong>을 클릭하시면 설정마법사를 계속 진행합니다.',
        defaultUserChangePass: '<strong>기본 사용자의 암호가 변경되어야 합니다!</strong>',
        defaultUserDisabled: '<strong>기본 사용자가 비활성화되었거나 Umbraco에 접근할 수 없습니다!</strong></p><p>더 이상 과정이 필요없으시면 <strong>다음</strong>을 눌러주세요.',
        defaultUserPassChanged: '<strong>설치후 기본사용자의 암호가 성공적으로 변경되었습니다!</strong></p><p>더 이상 과정이 필요없으시면 <strong>다음</strong>을 눌러주세요.',
        ...
        permissionsAlmostPerfect: '<strong>권한 설정이 대부분 완벽합니다!</strong><br /><br />\n      여러분은 문제없이 Umbraco사용이 가능하지만 일부 추천 패키지가 설치되지 않을 수 있습니다.',
        permissionsHowtoResolve: '문제해결방법',
        permissionsHowtoResolveLink: '문서버전을 읽으시려면 클릭하세요',
        permissionsHowtoResolveText: 'Umbraco를 위한 폴더권한세팅을 위해 텍스트 버전을 읽으시거나 저희 <strong>Video tutorial</strong>를 시청하세요.',
        permissionsMaybeAnIssue: '<strong>퍼미션 세팅에 문제가 있을 수 있습니다.</strong>\n    <br/><br />\n    Umbraco를 문제없이 실행할 수 있지만, 폴더를 만들거나 추천패키지를 설치하지 못할 수 있습니다.',
        permissionsNotReady: '<strong>권한 설정이 완료되지 않았습니다!</strong>\n        <br /><br />\n        Umbraco 실행을 위해, 권한설정을 업데이트하세요.',
        permissionsPerfect: '<strong>권한세팅이 완벽합니다!</strong><br /><br />\n            Umbraco 패키지 설치를 진행할 준비가 되었습니다. ',
    ...
    packager: {
        ...
        packageNoItemsText: '이 패키지엔 삭제할 아이템이 포함되어 있지 않습니다.<br/><br/>\n    아래 "패키지 삭제"를 클릭하시면 안전하게 시스템에서 삭제하실 수 있습니다.',
        ...
        packageUninstallText: '삭제하시려는 항목은 선택하지 않을 수 있습니다. "삭제 확인" 버튼을 누를때 체크되어있지 항목은 모두 삭제됩니다.<br />\n    <span style="color: Red; font-weight: bold;">알림:</span> 문서, 미디어등 삭제항목에 관련된 모든 항목이 삭제됩니다, 작업을 중단하면 시스템이 불안정적으로 동작할 수 있습니다.\n    삭제는 매우 주의를 요하기 때문에 의심스러운항목은 패키지 제작자에게 문의하시기 바랍니다.',
        packageVersion: '패키지 버전',
    },
    paste: {
        doNothing: '포맷을 포함여하 붙여넣기(권장하지 않음)',
        errorMessage: '붙여넣으려는 텍스트에 특수한 문자나 포맷이 포함되어있습니다. Microsoft Word문서에서 바로 복사해와서 문제가 발생된것일 수 있습니다. Umbraco는 붙여넣으려는 컨텐츠가 웹에 적합하도록 특수한 문자나 포맷을 자동으로 제거합니다',
    ...
    publish: {
        ...
        publishHelp: "<strong>%0%</strong>를 발행하기위해 <em>확인</em>를 클릭하세요 and thereby making it's content publicly available.<br/><br />\n    이 페이지와 모든 서브페이지를 아래 <em>모든 자식문서 발행</em>을 체크하여 발행할 수 있습니다.\n    ",
    },
    relatedlinks: {
        addExternal: '외부링크 추가',
        addInternal: '내부링크 추가',
    ...
    rollback: {
        diffHelp: '현재 버전과 선택한 버전의 차이점을 보여줍니다<br /><del>빨간</del> 텍스트는 선택한 버전에선 보이지 않습니다. <ins>녹색은 추가되었음을 의미합니다</ins>',
    ...
    settings: {
        ...
        importDocumentTypeHelp: '문서를 가져오시려면 사용하시는 컴퓨터에 ".udt"를 찾아 선택하시고 "가져오기"를 클릭하세요(다음 단계에서 확인여부를 문의합니다)',
        newtabname: '새 색인 제목',
        nodetype: '노드타입',
        objecttype: '타입',
        stylesheet: '스타일시트',
    ...
    sort: {
        ...
        sortDone: '정렬 완료',
        sortHelp: '다른 아이템을 마우스로 위,아래로 드래그 하여 이동하거나 열의 헤더를 클릭하여 아이템을 정렬할 수 있습니다',
    ...
    translation: {
        ...
        translationDoneHelp: '아래를 클릭하셔서 방금 번역한 페이지를 미리볼 수 있습니다. 원본 페이지가 있다면 두 페이지를 비교해보시기 바랍니다.',
