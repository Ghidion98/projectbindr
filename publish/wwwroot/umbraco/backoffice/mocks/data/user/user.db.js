...
const userGroupFilter = (filterOptions, item) => objectArrayFilter(filterOptions.userGroupIds, item.userGroupIds, 'id');
...
class UmbUserMockDB extends UmbEntityMockDbBase {
    constructor(data) {
        super(data);
    ...
    enableMfaProvider(providerName) {
    ...
    disableMfaProvider(providerName) {
        const provider = mfaLoginProviders.find((x) => x.providerName === providerName);
        if (provider) {
    ...
    filter(options) {
        ...
        const filteredItems = allItems.filter((item) => userGroupFilter(filterOptions, item) &&
