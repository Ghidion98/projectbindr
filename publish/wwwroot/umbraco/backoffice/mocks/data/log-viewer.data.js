...
class UmbLogViewerSearchesData extends UmbMockDBBase {
    constructor(data) {
        super(data);
    }
    // skip can be number or null
    getSavedSearches(skip = 0, take = this.data.length) {
...
class UmbLogViewerTemplatesData extends UmbMockDBBase {
    constructor(data) {
        super(data);
    }
    // skip can be number or null
    getTemplates(skip = 0, take = this.data.length) {
        return this.data.slice(skip, take + skip);
    }
}
class UmbLogViewerMessagesData extends UmbMockDBBase {
    constructor(data) {
        super(data);
    }
    // skip can be number or null
    getLogs(skip = 0, take = this.data.length) {
...
export const messageTemplates = [
    ...
    {
        messageTemplate: "Umbraco Forms scheduled record deletion task will not run as it has been not enabled via configuration. To enable, set the configuration value at 'Umbraco:Forms:Options:ScheduledRecordDeletion:Enabled' to true.",
        count: 5,
    },
    {
        messageTemplate: 'Mapped Umbraco.Cloud.Deployment.SiteExtension.Messages.External.Git.ApplyChangesFromWwwRootToRepositoryCommand -> "siteext-input-queue"',
        count: 3,
    },
    {
    ...
    {
        messageTemplate: "Found single permission letter for '{LegacyPermission}' on user group '{UserGroupAlias}', assuming this is the 'Notifications' permission instead of the Deploy 'Queue for transfer' permission",
        count: 3,
    },
    {
    ...
    {
        messageTemplate: "File system watcher for deploy events started with filter 'deploy*' and notify filter 'FileName'.",
        count: 3,
    },
    {
    ...
    {
        messageTemplate: 'Database migration step not completed: could not create primary key constraint on UFRecordDataLongString as a primary key already exists.',
        count: 2,
    },
    {
        messageTemplate: 'No last synced Id found, this generally means this is a new server/install. A cold boot will be triggered.',
        count: 2,
    },
    {
    ...
    {
        messageTemplate: 'Registered with MainDom, localContentDbExists? {LocalContentDbExists}, localMediaDbExists? {LocalMediaDbExists}',
        count: 2,
    },
    {
    ...
    {
        messageTemplate: 'Database migration step not completed: could not create primary key constraint on UFRecordDataBit as a primary key already exists.',
        count: 1,
    },
    {
        messageTemplate: 'Database migration step not completed: could not create primary key constraint on UFRecordDataDateTime as a primary key already exists.',
        count: 1,
    },
    {
        messageTemplate: 'Database migration step not completed: could not create primary key constraint on UFRecordDataInteger as a primary key already exists.',
        count: 1,
    },
    {
        messageTemplate: 'Database migration step not completed: could not create primary key constraint on UFUserFormSecurity as a primary key already exists.',
        count: 1,
    },
    {
        messageTemplate: 'Database migration step not completed: could not create primary key constraint on UFUserGroupSecurity as a primary key already exists.',
        count: 1,
    },
    {
        messageTemplate: 'Database migration step not completed: could not create primary key constraint on UFUserGroupFormSecurity as a primary key already exists.',
        count: 1,
    },
    {
        messageTemplate: 'Database NuCache was serialized using {CurrentSerializer}. Currently configured NuCache serializer {Serializer}. Rebuilding Nucache',
        count: 1,
    },
    {
