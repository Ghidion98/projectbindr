...
export const handlers = [
    rest.get(umbracoPath('/security/back-office/graphics/logo'), async (req, res, ctx) => {
        ...
        return res(ctx.set('Content-Length', imageBuffer.byteLength.toString()), ctx.set('Content-Type', 'image/svg+xml'), ctx.body(imageBuffer));
    }),
    ...
    rest.get(umbracoPath('/security/back-office/graphics/login-background'), async (req, res, ctx) => {
        ...
        return res(ctx.set('Content-Length', imageBuffer.byteLength.toString()), ctx.set('Content-Type', 'image/jpeg'), ctx.body(imageBuffer));
    }),
];
