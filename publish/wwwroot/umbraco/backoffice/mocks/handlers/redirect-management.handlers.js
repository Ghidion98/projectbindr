    ...
    rest.get(umbracoPath('/redirect-management'), (_req, res, ctx) => {
        ...
        if (filter) {
            ...
            PagedRedirectUrlData.items.forEach((item) => {
                if (item.originalUrl?.includes(filter))
                    filtered.push(item);
            });
            const filteredPagedData = {
...
// Mock Data
const UrlTracker = { status: RedirectStatusModel.ENABLED, userIsAdmin: true };
const _getRedirectUrlByKey = (id) => {
    const PagedResult = {
        total: 0,
        items: [],
    };
...
const _deleteRedirectUrlByKey = (id) => {
    const index = RedirectUrlData.findIndex((data) => data.id === id);
    if (index > -1)
        RedirectUrlData.splice(index, 1);
    const PagedResult = {
